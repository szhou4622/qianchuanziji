...
所有来自千川的金额/ROI/出价/预算以“官方字段原单位的规范 Decimal 字符串”存 TEXT，
禁止在 Schema 层未经官方契约证明就换算为分、厘等整数单位。
"""
...
CREATE TABLE IF NOT EXISTS qianchuan_account(account_uid TEXT PRIMARY KEY,advertiser_id TEXT NOT NULL UNIQUE,account_name TEXT,account_type TEXT,enabled INTEGER NOT NULL DEFAULT 0 CHECK(enabled IN(0,1)),auth_status TEXT NOT NULL,capability_json TEXT,last_auth_ok_at TEXT,last_seen_at TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS material_latest(material_uid TEXT PRIMARY KEY REFERENCES material_registry(material_uid),advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,material_id TEXT NOT NULL,official_material_status TEXT,official_audit_status TEXT,overall_cost_decimal TEXT,net_settle_amount_decimal TEXT,net_settle_roi_decimal TEXT,net_settle_order_count INTEGER,overall_order_count INTEGER,overall_gmv_decimal TEXT,overall_pay_roi_decimal TEXT,stat_date TEXT NOT NULL,collected_at TEXT NOT NULL,batch_id TEXT NOT NULL REFERENCES collection_batch(batch_id),request_id TEXT,sync_state TEXT NOT NULL,strategy_eligible INTEGER NOT NULL DEFAULT 0 CHECK(strategy_eligible IN(0,1)),updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS material_5m(snapshot_id TEXT PRIMARY KEY,material_uid TEXT NOT NULL REFERENCES material_registry(material_uid),advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,material_id TEXT NOT NULL,scheduled_at TEXT NOT NULL,collected_at TEXT NOT NULL,stat_date TEXT NOT NULL,overall_cost_decimal TEXT,net_settle_amount_decimal TEXT,net_settle_roi_decimal TEXT,net_settle_order_count INTEGER,overall_order_count INTEGER,overall_gmv_decimal TEXT,overall_pay_roi_decimal TEXT,official_material_status TEXT,official_audit_status TEXT,batch_id TEXT NOT NULL REFERENCES collection_batch(batch_id),snapshot_reason TEXT NOT NULL,UNIQUE(material_uid,scheduled_at,snapshot_reason));
CREATE INDEX IF NOT EXISTS idx_material_5m_object_time ON material_5m(material_uid,scheduled_at DESC);
CREATE TABLE IF NOT EXISTS material_daily(advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,material_id TEXT NOT NULL,business_date TEXT NOT NULL,overall_cost_decimal TEXT,net_settle_amount_decimal TEXT,net_settle_roi_decimal TEXT,net_settle_order_count INTEGER,overall_order_count INTEGER,overall_gmv_decimal TEXT,overall_pay_roi_decimal TEXT,data_status TEXT NOT NULL,revision_no INTEGER NOT NULL DEFAULT 1,finalized_at TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),request_id TEXT,updated_at TEXT NOT NULL,PRIMARY KEY(advertiser_id,ad_id,material_id,business_date));
CREATE INDEX IF NOT EXISTS idx_material_daily_plan_date ON material_daily(advertiser_id,ad_id,business_date DESC);
CREATE TABLE IF NOT EXISTS material_monthly(advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,material_id TEXT NOT NULL,business_month TEXT NOT NULL,overall_cost_decimal TEXT,net_settle_amount_decimal TEXT,net_settle_roi_decimal TEXT,net_settle_order_count INTEGER,overall_order_count INTEGER,overall_gmv_decimal TEXT,overall_pay_roi_decimal TEXT,data_status TEXT NOT NULL,revision_no INTEGER NOT NULL DEFAULT 1,finalized_at TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),request_id TEXT,updated_at TEXT NOT NULL,PRIMARY KEY(advertiser_id,ad_id,material_id,business_month));

CREATE TABLE IF NOT EXISTS material_topic_metric_latest(topic_metric_uid TEXT PRIMARY KEY,advertiser_id TEXT NOT NULL,plan_system TEXT NOT NULL,promotion_scene TEXT NOT NULL,material_id TEXT NOT NULL,stat_date TEXT NOT NULL,topic_name TEXT NOT NULL,basic_cost_decimal TEXT,comprehensive_marketing_roi_decimal TEXT,collected_at TEXT NOT NULL,request_id TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),UNIQUE(advertiser_id,plan_system,promotion_scene,material_id,stat_date,topic_name));
CREATE TABLE IF NOT EXISTS material_topic_metric_daily(advertiser_id TEXT NOT NULL,plan_system TEXT NOT NULL,promotion_scene TEXT NOT NULL,material_id TEXT NOT NULL,business_date TEXT NOT NULL,topic_name TEXT NOT NULL,basic_cost_decimal TEXT,comprehensive_marketing_roi_decimal TEXT,data_status TEXT NOT NULL,revision_no INTEGER NOT NULL DEFAULT 1,finalized_at TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),request_id TEXT,updated_at TEXT NOT NULL,PRIMARY KEY(advertiser_id,plan_system,promotion_scene,material_id,business_date,topic_name));
CREATE TABLE IF NOT EXISTS material_topic_metric_monthly(advertiser_id TEXT NOT NULL,plan_system TEXT NOT NULL,promotion_scene TEXT NOT NULL,material_id TEXT NOT NULL,business_month TEXT NOT NULL,topic_name TEXT NOT NULL,basic_cost_decimal TEXT,comprehensive_marketing_roi_decimal TEXT,data_status TEXT NOT NULL,revision_no INTEGER NOT NULL DEFAULT 1,finalized_at TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),request_id TEXT,updated_at TEXT NOT NULL,PRIMARY KEY(advertiser_id,plan_system,promotion_scene,material_id,business_month,topic_name));
...
CREATE TABLE IF NOT EXISTS control_task_latest(control_task_uid TEXT PRIMARY KEY REFERENCES control_task_registry(control_task_uid),advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,control_task_id TEXT NOT NULL,official_task_status TEXT,budget_decimal TEXT,duration_decimal TEXT,bid_decimal TEXT,roi_goal_decimal TEXT,assist_cost_decimal TEXT,assist_order_count INTEGER,assist_gmv_decimal TEXT,assist_pay_roi_decimal TEXT,assist_net_amount_decimal TEXT,assist_net_roi_decimal TEXT,assist_net_order_count INTEGER,stat_start_time TEXT,stat_end_time TEXT,stat_date TEXT NOT NULL,collected_at TEXT NOT NULL,batch_id TEXT NOT NULL REFERENCES collection_batch(batch_id),request_id TEXT,sync_state TEXT NOT NULL,strategy_eligible INTEGER NOT NULL DEFAULT 0 CHECK(strategy_eligible IN(0,1)),write_eligible INTEGER NOT NULL DEFAULT 0 CHECK(write_eligible IN(0,1)),updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS control_task_5m(snapshot_id TEXT PRIMARY KEY,control_task_uid TEXT NOT NULL REFERENCES control_task_registry(control_task_uid),advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,control_task_id TEXT NOT NULL,scheduled_at TEXT NOT NULL,stat_start_time TEXT,stat_end_time TEXT,stat_date TEXT NOT NULL,assist_cost_decimal TEXT,assist_order_count INTEGER,assist_gmv_decimal TEXT,assist_pay_roi_decimal TEXT,assist_net_amount_decimal TEXT,assist_net_roi_decimal TEXT,assist_net_order_count INTEGER,official_task_status TEXT,batch_id TEXT NOT NULL REFERENCES collection_batch(batch_id),snapshot_reason TEXT NOT NULL,UNIQUE(control_task_uid,scheduled_at,snapshot_reason));
CREATE INDEX IF NOT EXISTS idx_control_5m_task_time ON control_task_5m(control_task_uid,scheduled_at DESC);
CREATE TABLE IF NOT EXISTS control_task_daily(advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,control_task_id TEXT NOT NULL,business_date TEXT NOT NULL,assist_cost_decimal TEXT,assist_order_count INTEGER,assist_gmv_decimal TEXT,assist_pay_roi_decimal TEXT,assist_net_amount_decimal TEXT,assist_net_roi_decimal TEXT,assist_net_order_count INTEGER,data_status TEXT NOT NULL,revision_no INTEGER NOT NULL DEFAULT 1,finalized_at TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),request_id TEXT,updated_at TEXT NOT NULL,PRIMARY KEY(advertiser_id,ad_id,control_task_id,business_date));
CREATE TABLE IF NOT EXISTS control_task_monthly(advertiser_id TEXT NOT NULL,ad_id TEXT NOT NULL,control_task_id TEXT NOT NULL,business_month TEXT NOT NULL,assist_cost_decimal TEXT,assist_order_count INTEGER,assist_gmv_decimal TEXT,assist_pay_roi_decimal TEXT,assist_net_amount_decimal TEXT,assist_net_roi_decimal TEXT,assist_net_order_count INTEGER,data_status TEXT NOT NULL,revision_no INTEGER NOT NULL DEFAULT 1,finalized_at TEXT,batch_id TEXT REFERENCES collection_batch(batch_id),request_id TEXT,updated_at TEXT NOT NULL,PRIMARY KEY(advertiser_id,ad_id,control_task_id,business_month));
