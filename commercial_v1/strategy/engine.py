...
只消费 Phase 3 已持久化的 TRUSTED Latest；不访问千川网络、不创建候选、不执行平台
写操作。任何 NULL/缺字段都进入 NOT_EVALUABLE，绝不猜成 0。
"""
...
def _decimal_text(value: Any, *, label: str) -> str:
    rendered = format(_decimal(value, label=label), "f")
    if "." in rendered:
        rendered = rendered.rstrip("0").rstrip(".")
    return rendered or "0"


def _scope(target_uid: str) -> str:
...
def _normalize_conditions(strategy_type: str, spec: Mapping[str, Any]) -> dict[str, Any]:
    contract = STRATEGY_CONTRACTS.get(str(strategy_type or "").strip().upper())
    if contract is None:
        raise ValueError("unsupported V1 strategy_type")
    if str(spec.get("logic") or "").strip().upper() != "AND":
        ...
        normalized.append(
            {
                "field": field,
                "op": op,
                "value": _decimal_text(raw["value"], label=f"condition[{index}].value"),
            }
        )
...
def _hit_id(version_id: str, object_uid: str, batch_id: str) -> str:
    raw = f"{version_id}|{object_uid}|{batch_id}".encode("utf-8")
...
class StrategyStore:
    ...
    def create_strategy(
        ...
        *,
        ...
        target_scope = _scope(target_uid)
        ...
        def work(conn):
            if conn.execute("SELECT 1 FROM monitor_plan WHERE target_uid=?", (target_uid,)).fetchone() is None:
            ...
            conn.execute(
                ...
                (strategy_id, name, kind, target_scope, action_type, mode, prio, version_id, now, now),
            )
            conn.execute(
    ...
    def create_new_version(
        ...
        *,
        ...
        next_action = _normalize_action_config(action_config) if action_config is not None else dict(current.action_config)
        grouping = current.grouping_mode if grouping_mode is None else str(grouping_mode).strip().upper()
        if grouping not in GROUPING_MODES:
            raise ValueError("grouping_mode must be SEPARATE or MERGED")
        ...
        def work(conn):
            row = conn.execute(
                "SELECT current_version_id FROM strategy_config WHERE strategy_id=?", (strategy_id,)
            ).fetchone()
            if row is None:
                raise ValueError("strategy does not exist")
            if str(row["current_version_id"] or "") != current.strategy_version_id:
        ...
        with self._database.connect(readonly=True) as conn:
            row = conn.execute(
                """SELECT c.*,v.strategy_version_id,v.version_no,v.condition_json,v.action_config_json,
                   ...
                   FROM strategy_config c JOIN strategy_version v
                     ON v.strategy_version_id=c.current_version_id
                   WHERE c.strategy_id=?""",
                (strategy_id,),
            ).fetchone()
        if row is None:
    ...
    def list_for_target(self, target_uid: str, *, object_type: str) -> tuple[StrategyVersion, ...]:
        ...
        values = tuple(self._row_to_version(row) for row in rows)
        return tuple(value for value in values if value.object_type == wanted)
...
class StrategyEvaluator:
    @staticmethod
    def evaluate(version: StrategyVersion, metrics: Mapping[str, Any]) -> tuple[str, tuple[ConditionOutcome, ...]]:
        allowed = STRATEGY_CONTRACTS[version.strategy_type][2]
        ...
        for condition in version.conditions["conditions"]:
            ...
            if raw is None:
                ...
                try:
                    actual = _decimal(raw, label=field)
                    ...
                    any_not_hit = any_not_hit or not matched
            outcomes.append(ConditionOutcome(field, op, expected_text, actual_text, result))
        if any_not_hit:
...
class StrategyEvaluationService:
    def __init__(self, database: Database, writer: StorageWriter, store: StrategyStore) -> None:
        self._database = database
        self._writer = writer
        self._store = store
    ...
    def _evaluate_batch(self, target_uid: str, source_batch_id: str, *, object_type: str) -> EvaluationSummary:
        target = self._load_target(target_uid)
        batch = self._load_batch(target_uid, source_batch_id, object_type)
        ...
        for row in objects:
            ...
            for version in strategies:
                result, conditions = evaluator.evaluate(version, metrics)
                outcomes.append(
                    EvaluationOutcome(
        ...
        return EvaluationSummary(
            ...
            hit=sum(item.result == HIT for item in arbitrated),
            not_hit=sum(item.result == NOT_HIT for item in arbitrated),
            not_evaluable=sum(item.result == NOT_EVALUABLE for item in arbitrated),
            persisted_hits=persisted,
            suppressed_hits=sum(bool(item.suppression_reason) for item in arbitrated),
    ...
    def _load_batch(self, target_uid: str, batch_id: str, object_type: str) -> Mapping[str, Any]:
        pipeline = "MATERIAL_5M" if object_type == "MATERIAL" else "CONTROL_5M"
        with self._database.connect(readonly=True) as conn:
            row = conn.execute(
                """SELECT * FROM collection_batch WHERE batch_id=? AND target_uid=?
                   AND pipeline_type=? AND status='SUCCESS'""",
                (batch_id, target_uid, pipeline),
            ).fetchone()
        if row is None:
            raise ValueError("source batch is not a trusted successful hot batch")
        return row

    def _load_objects(self, target: Mapping[str, Any], batch_id: str, object_type: str) -> tuple[Mapping[str, Any], ...]:
        with self._database.connect(readonly=True) as conn:
            if object_type == "MATERIAL":
                rows = conn.execute(
                    """SELECT * FROM material_latest WHERE advertiser_id=? AND ad_id=? AND batch_id=?
                       AND sync_state='TRUSTED' AND strategy_eligible=1
                       AND official_material_status='DELIVERY_OK' ORDER BY material_uid""",
                    (target["advertiser_id"], target["ad_id"], batch_id),
                ).fetchall()
            else:
                rows = conn.execute(
                    """SELECT * FROM control_task_latest WHERE advertiser_id=? AND ad_id=? AND batch_id=?
                       AND sync_state='TRUSTED' AND strategy_eligible=1
                       AND official_task_status='PROCESSING' ORDER BY control_task_uid""",
                    (target["advertiser_id"], target["ad_id"], batch_id),
                ).fetchall()
        return tuple(rows)
    ...
    def _arbitrate(outcomes: Sequence[EvaluationOutcome]) -> list[EvaluationOutcome]:
        ...
        for index, outcome in enumerate(values):
            if outcome.result == HIT:
                groups.setdefault((outcome.object_uid, outcome.action_type), []).append(index)
        for indices in groups.values():
            ordered = sorted(indices, key=lambda index: (-values[index].priority, values[index].strategy_id))
            winner_index = ordered[0]
            winner = values[winner_index]
            values[winner_index] = replace(
                winner,
                hit_id=_hit_id(winner.strategy_version_id, winner.object_uid, winner.source_batch_id),
            ...
            for index in ordered[1:]:
                ...
                values[index] = replace(
                    loser,
                    hit_id=_hit_id(loser.strategy_version_id, loser.object_uid, loser.source_batch_id),
    ...
    def _persist_hits(self, target_uid: str, outcomes: Sequence[EvaluationOutcome]) -> int:
        hits = [item for item in outcomes if item.result == HIT]
        if not hits:
            return 0
        ...
        def work(conn):
            ...
            for outcome in hits:
                ...
                cursor = conn.execute(
                    ...
                    (
                        outcome.hit_id or _hit_id(outcome.strategy_version_id, outcome.object_uid, outcome.source_batch_id),
                        outcome.strategy_id,
